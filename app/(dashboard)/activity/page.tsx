  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <FileDownload href="/api/activity/export" className="ticket rounded-[4px] px-3 py-2 text-sm">
            Export CSV
          </FileDownload>
        }
      />
                  ...
                  <tr key={event.id} className="border-t">
                    ...
                    <td>
                      <span className={`inline-flex items-center gap-1.5 capitalize ${kind === "ai" ? "text-cyan" : kind === "human" ? "text-amber" : "text-muted"}`}>
                        <span className={`size-1.5 rounded-full ${kind === "ai" ? "bg-cyan" : kind === "human" ? "bg-amber" : "bg-muted"}`} />
                        {event.actorType}
                      </span>
                    </td>
